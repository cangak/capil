...
class Crud {
	...
	public function colom() {
		return $this->db->query("SHOW TABLES FROM 2016_focapil ");
	}
	public function kolom($x) {
		return $this->db->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '" . $x . "'");
	}
	public function field($x) {
		return $this->db->query("SHOW FIELDS FROM " . $x);
	}

	public function all_order($order, $urutan) {
//        	return $this->db->query("SELECT * FROM " . $this->table . " order by position asc");
		return $this->db->query("SELECT * FROM " . $this->table . " order by " . $order . " " . $urutan);
		// echo "SELECT * FROM " . $this->table . " order by ". $order ." ".$urutan;
	}
